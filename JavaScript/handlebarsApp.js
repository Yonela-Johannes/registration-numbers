...
const towns = document.querySelector('#handlebars-regno')
const townName = document.querySelector('.handlebars-display-name')
const filteredNumberPlate = document.querySelector('.handlebars-filter-plate-inner-box')
const errorHandler = document.querySelector('.handlebars-error-handling')
const errorHandlerWrapper = document.querySelector('.handlebars-error-handling-wrapper')
const regInput = document.querySelector('.handlebars-reg-input')
const addNumber = document.querySelector('.handlebars-add-number')
const resetRegNumbers = document.querySelector('.handlebars-reset-numbers')
const counter = document.querySelector(".handlebars-counter")
// get/set local storage
JSON.parse(localStorage.getItem('handlebarsRegNumbers')) == null && localStorage.setItem("handlebarsRegNumbers", JSON.stringify([]))
let numbers = JSON.parse(localStorage.getItem('handlebarsRegNumbers')).reverse()
    ...
    if (!regNumbers.limit()) {
        if (regNumbers.getRegNumber() && regNumbers.validNo() && regNumbers.checkReg()) {
            ...
            if (numbers.includes(regNumbers.getRegNumber()) == false) {
                ...
                localStorage.setItem("handlebarsRegNumbers", JSON.stringify(numbers))
                storedRegis(filteredNumberPlate, numbers)
...
const clearRegNumbers = () => {
    !!regNumbers.getNumbers() && (errorHandlerWrapper.classList.remove("hide")),
        errorHandler.classList.add('fail'),
        regNumbers.getNumbers().length == 0 ? errorHandler.innerHTML = 'Registration numbers are already cleared' : localStorage.removeItem('handlebarsRegNumbers')
    setTimeout(() => {
